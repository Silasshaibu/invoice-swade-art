...
export async function GET(req: NextRequest) {
  ...
  try {
    await initDB()
    logs.push('Database initialized successfully')

    const dbUrl = process.env.DATABASE_URL
    logs.push(`DATABASE_URL is ${dbUrl ? 'present' : 'missing'}`)

    // Find the admin user
    const users = await sql`SELECT id FROM users WHERE email = 'silasshaibu2@gmail.com' LIMIT 1`
    if (users.length === 0) {
      logs.push('Admin user silasshaibu2@gmail.com NOT found!')
      return Response.json({ success: false, logs })
    }
    const userId = users[0].id
    logs.push(`Found admin user silasshaibu2@gmail.com with ID: ${userId}`)

    // Check all users in db to help debug
    const allUsers = await sql`SELECT id, email FROM users`
    logs.push(`All users in DB: ${JSON.stringify(allUsers)}`)
    ...
    if (existingClients.length > 0) {
      ...
      logs.push(`Found existing client Marco Martinez with ID: ${client.id}`)
    } else {
      ...
      `
      client = newClients[0]
      logs.push(`Created new client Marco Martinez with ID: ${client.id}`)
    }
    ...
    if (existingInvoices.length > 0) {
      logs.push('Invoice number 98122 already exists. Updating it or skipping.')
    } else {
      // Insert invoice
      ...
      logs.push(`Inserted invoice with ID: ${invoiceId}`)
      ...
      `
      logs.push('Inserted invoice line items successfully.')
    }

    return Response.json({ success: true, logs })
  } catch (error: any) {
    logs.push(`Error: ${error.message || String(error)}`)
    return Response.json({ success: false, logs, error: String(error) }, { status: 500 })
  }
}
